...
window_size = int(np.sqrt(days*1000) * 100)
season = 0

# define the prices candidates for the first and second item
candidates_item1 = [1900.0, 2130.0, 1920.0, 2340.0]
candidates_item2 = [365.0, 400.0, 550.0, 380.0]
...
for e in range(n_exp):
    ...
    matching_swts_learners = [promo_category_UCB_CD_learner(np.zeros((4,4)).size, *np.zeros((4,4)).shape, M=10, eps=2.30231861e-02, h=6.64000000e+02, alpha=4.71117966e-02,starting_delay=400,normalizing_value=max(candidates_item2)) for _ in range(len(candidates_item1) * len(candidates_item2))]
    matching_ts_learners = [promo_category_UCB_CD_learner(np.zeros((4,4)).size, *np.zeros((4,4)).shape, M=30, eps=2.30231861e-02, h=6.64000000e+02, alpha=4.71117966e-02,starting_delay=400,normalizing_value=max(candidates_item2)) for _ in range(len(candidates_item1) * len(candidates_item2))]
    v_daily_swts_reward = []
    v_daily_ts_reward = []
    v_daily_opt_reward = []
...
plt.xlabel("Days")
plt.ylabel("Regret")
plt.plot(np.mean(v_swts_experimets,axis=0),'-', color='darkorange', label = 'SWTS-CD')
plt.plot(np.mean(v_ts_experimets,axis=0),'-', color='blue', label = 'TS-CD')
