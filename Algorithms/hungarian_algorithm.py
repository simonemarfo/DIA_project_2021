...
ha_values_per_experiments = []
scipy_values_per_experiments = []

for i in range(0,n_experiments):
    a = np.random.randint(100, size=(8, 8))
    print("\n # ", i,"\n" )
    #implemented hungarian algorithm
    res = hungarian_algorithm(a)
    print("\nHungarian Algorithm\n\nMatrix:\n", a, "\nOptimal Matching:\n", res[1], "\nValue: ", np.sum(res[0]))
    ha_values_per_experiments.append(np.sum(res[0]))

    # scipy solution 
    row_ind,col_ind = linear_sum_assignment(a)
    print("\nScipy linear sum assignment\n\nMatrix:\n", a, "\nOptimal Matching:\n",row_ind,col_ind, "\nValue: ", a[row_ind,col_ind].sum())
    scipy_values_per_experiments.append(a[row_ind,col_ind].sum())

plt.figure(0)
plt.xlabel("number_of_experiment")
plt.ylabel("Value")
plt.plot(ha_values_per_experiments, 'r')
plt.plot(scipy_values_per_experiments, 'g')
...
plt.xlabel("number_of_experiment")
plt.plot(np.divide(ha_values_per_experiments,scipy_values_per_experiments) ,marker = '.', linestyle='None', color='b')
plt.legend(['Hungarian < 1, Scipy > 1'])
