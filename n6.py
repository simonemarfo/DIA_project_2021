...
for e in range(n_exp):
    ...
    for d in range(days):
        ...
        for customer in range(tot_client):
            ...
            opt_customer_item1 = 0.0
            opt_customer_item2 = 0.0
            ...
            print(f'|\t[TS] - {ctx.items_info[0]["name"]} : {candidates_item1[ts_pulled_arm_item1]} €, {ctx.items_info[1]["name"]} : {candidates_item2[ts_pulled_arm_item2]} € -> Total reward : {round(customer_reward_item1,2)} € Item2 reward : {round(customer_reward_item2,2)}')
            print(f'|\t[OPT] - {ctx.items_info[0]["name"]} : {candidates_item1[optimal_candidates1[category]]} €, {ctx.items_info[1]["name"]} : {candidates_item2[optimal_candidates2[category]]} € -> Total reward : {round(opt_customer_item1,2)} € Item2 reward : {round(opt_customer_item2,2)}')
...
plt.xlabel("#sales")
plt.ylabel("Regret")
plt.plot(np.mean(experiments,axis=0),'-', color='yellow', label = 'Total regret')
