    ...
    for d in range(days):
        ...
        for customer in range(tot_client):
            ...
            if ts_buy_or_not_item1:
                ts_reward += candidates_item1[ts_pulled_arm] + ctx.purchase_online_second_element(discounted_price[category],category)*discounted_price[category]
            if ucb_buy_or_not_item1:
                ucb_reward += candidates_item1[ucb_pulled_arm] + ctx.purchase_online_second_element(discounted_price[category],category)*discounted_price[category]
            if (opt_buy_or_not_item1):
                opt_reward += min(candidates_item1) + discounted_price[category]
