...
for e in range(n_exp):
    ...
    for t in range(days): # Day simulation
        day_cum_UCB_rewards = [] # rewards collected in a period (days) performing the online learning strategy
        day_cum_opt_rewards = [] # rewards collected in a period (days) performing the online learning strategy
        ...
        daily_customer = []
        daily_customer_copy = []
        daily_customer_weight = ctx.customers_daily_instance()

        daily_customer.append(ctx.customers_daily_instance())
        daily_customer.append(ctx.customers_daily_instance())
        daily_customer.append(ctx.customers_daily_instance())
        daily_customer.append(ctx.customers_daily_instance())
        daily_customer.append(ctx.customers_daily_instance())
        ...
        daily_customer_copy.append(daily_customer[0].copy())
        daily_customer_copy.append(daily_customer[1].copy())
        daily_customer_copy.append(daily_customer[2].copy())
        daily_customer_copy.append(daily_customer[3].copy())
        daily_customer_copy.append(daily_customer[4].copy())
        daily_customer_copy.append(daily_customer[5].copy())

        
        category=0
        for day_period in range(0,divisor):
            cum_UCB_rewards = 0
            cum_opt_rewards = 0
            tot_client=sum(daily_customer[day_period])
            ...
            for customer in range(tot_client): # for each category emulate the user that purchase the good 
                n_cli+=1
                category = np.random.choice(np.nonzero(daily_customer[day_period])[0])
                daily_customer[day_period][category] -= 1
            ...
            print(daily_customer_copy)
            if((t*divisor+day_period)<delay):
                rewards=[0,0,0,0]
                max_rew[0]=max(rewards_to_update[0]/daily_customer_copy[day_period][0],max_rew[0])
                max_rew[1]=max(rewards_to_update[1]/daily_customer_copy[day_period][1],max_rew[1])
                max_rew[2]=max(rewards_to_update[2]/daily_customer_copy[day_period][2],max_rew[2])
                max_rew[3]=max(rewards_to_update[3]/daily_customer_copy[day_period][3],max_rew[3])
            else:
                rewards[0]=round(rewards_to_update[0]/(daily_customer_copy[day_period][0]*max_rew[0]),decimal_digits)
                rewards[1]=round(rewards_to_update[1]/(daily_customer_copy[day_period][1]*max_rew[1]),decimal_digits)
                rewards[2]=round(rewards_to_update[2]/(daily_customer_copy[day_period][2]*max_rew[2]),decimal_digits)
                rewards[3]=round(rewards_to_update[3]/(daily_customer_copy[day_period][3]*max_rew[3]),decimal_digits)
            ...
            day_cum_UCB_rewards.append(cum_UCB_rewards)
            day_cum_opt_rewards.append(cum_opt_rewards)
            ...
            print('___________________\n')
        period_UCB_reward.append(sum(day_cum_UCB_rewards))
        period_opt_reward.append(sum(day_cum_opt_rewards))
